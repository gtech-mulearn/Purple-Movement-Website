...
type PopupCardProps = {
  ...
  const PopupCard = ({ title, message, }: PopupCardProps) => {
    const [showCard, setShowCard] = useState(true);
    return (
        <>  
        {showCard && (
        <div className="absolute top-24 md:left-[30rem]  md:right-[35rem] mt-8 mx-4 md:mx-0 bg-[url('../src/assets/images/ptm.jpg')] bg-cover bg-center border shadow-lg rounded-xl p-4 z-10 h-48 w-74 md:w-1/3 ">
            <button className="absolute top-4 right-4 text-gray-500 hover:text-gray-700" aria-label="Close" onClick={() => setShowCard(false) } >
